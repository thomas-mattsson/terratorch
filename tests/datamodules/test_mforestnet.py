...
def dummy_forestnet_data(tmp_path) -> str:
    ...
    bands_order = MForestNetNonGeo.all_band_names
    ...
    for sample_id in ["sample_train", "sample_valid", "sample_test"]:
        ...
        create_dummy_h5_pickle(str(file_path), keys, dummy_shape, label=0)
    return str(base_dir)

def test_forestnet_datamodule(dummy_forestnet_data):
    from terratorch.datamodules import MForestNetNonGeoDataModule

    bands = MForestNetNonGeo.all_band_names
    datamodule = MForestNetNonGeoDataModule(
