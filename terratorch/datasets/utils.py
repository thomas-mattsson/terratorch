...
def to_rgb(image_chw: np.ndarray, rgb_indices: list[int]) -> np.ndarray:
    ...
    ch_min = img.min(axis=(0, 1), keepdims=True)
    ch_max = np.maximum(img.max(axis=(0, 1), keepdims=True), 1e-8)

    img = (img - ch_min) * (1.0 / ch_max)
...
def to_pca_rgb(image_chw: np.ndarray, step: int = 4) -> np.ndarray:
    ...
    Args:
        ...
        step: Use every `step`-th spatial embedding for faster PCA fitting. Step 4 uses 1/4 of all embeddings.
    """
    if image_chw.ndim != 3:
        raise ValueError(f"Unsupported embedding shape {tuple(image_chw.shape)}")

    C, H, W = image_chw.shape
    emb_flat = image_chw.reshape(C, -1).T  # (N, C)
    ...
    proj = pca.transform(emb_flat.astype(np.float32, copy=False) - mean)  # (N, 3)
